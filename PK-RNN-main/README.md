...
## PyTorch LSTM + Attention Pipeline

The repository also includes a PyTorch implementation of an LSTM encoder with multi-head attention for vancomycin concentration forecasting. To try it with the bundled sample dataset:

```bash
pip install -r requirements.txt  # ensure PyTorch is available
python train_lstm_attention.py
```

The script mirrors the batching logic used by `PK_RNN`, saves the best weights to `best_lstm_attention.pt`, and prints epoch-level training/validation loss. Replace `dataset.Example` with your own dataset to train on real cohorts.
